	...
	<div class="table-responsive  vertical-center">
		<div class="table-wrapper ">
			<div class="table-title ">
				<div class="row">
					<div class="col-sm-6">
						<h2>Upravljaj <b>Rezervacijama</b></h2>
					</div>
					<div class="col-sm-6">
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr>
                        ...
                        <th>Grad preuzimanja</th>
                        <th>Status narudzbe</th>
                        @if (Auth::user()->isAdmin() or Auth::user()->isSuperAdmin())   <th>Potvrdi ili odbij</th>@endif
						</tr>
					<tbody>
				...
				@foreach ($rezervacije as $rezervacija)
					<tr>
						<td>{{$rezervacija->first_name}}</td>
                        <td>{{$rezervacija->last_name}}</td>
						<td>{{$rezervacija->email}}</td>
                       <td> {{$rezervacija->tel_number}}</td>
                       <td>{{$rezervacija->res_date}}</td>
                       <td>{{$rezervacija->city}}</td>
						<td> @if($rezervacija->status==0) Na čekanju
							@elseif($rezervacija->status==1)Potvrđeno
							@else ($rezervacija->status==0) Odbijeno
								@endif</td>
					...
					@if (Auth::user()->isAdmin() or Auth::user()->isSuperAdmin())
                        <td>
							...
							<a href="{{ route('potvrdi', $rezervacija->id) }}" ><button class="btn btn-primary m-1 btn-sm">Potvrdi Narudzbu</button></a>
                            <a href="{{ route('odbij', $rezervacija->id) }}" ><button class="btn btn-danger m-1 btn-sm">Odbij Narudzbu</button></a>
							</div>
	...
	<div class="modal-dialog">
		<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<a href=""><button class="btn btn-primary"><input type="submit" class="btn btn-danger" value="Izbriši"></a>
				</div>
